...
function send(connection, message) {
  if (!connection.socket.writable || connection.socket.destroyed) return false;
  ...
  try {
    return connection.socket.write(Buffer.concat([header, payload]));
  } catch {
    return false;
  }
}
  ...
  try {
    switch (event.type) {
      ...
      case "rejectRestart":
        ...
        break;
      case "forceRestartGame":
        forceRestartGame(connection);
        break;
      case "dissolveRoom":
        dissolveRoom(connection);
        break;
      case "attack":
...
function forceRestartGame(connection) {
  requireHost(connection);
  requireGame();
  startNewGame();
}

function dissolveRoom(connection) {
  requireHost(connection);
  room.seats = [];
  room.game = null;
  room.hostConnectionId = null;
  for (const item of room.connections.values()) {
    item.playerId = null;
    item.name = "";
  }
  broadcastAllViews();
...
function broadcastAllViews() {
  ...
  for (const connection of room.connections.values()) {
    if (!send(connection, viewFor(connection))) failedConnections.push(connection);
  }
  failedConnections.forEach((connection) => disconnect(connection));
}
...
function heartbeatConnections() {
  const now = Date.now();
  for (const connection of [...room.connections.values()]) {
    if (now - connection.lastSeenAt > STALE_CONNECTION_MS) {
      connection.socket.destroy();
      disconnect(connection);
      continue;
    }
    if (!send(connection, { type: "ping", t: now })) {
      disconnect(connection);
    }
  }
}
